"""
kelelium's API is contained in module ``kelelium``. It is a simple Python API that
makes specifying web automation cases as simple as describing them to someone
looking over their shoulder at a screen.

The public functions and classes of kelelium are listed below. If you wish to use
kelelium functions in your Python scripts you can import them from the
``kelelium`` module::

	from kelelium import *
"""
...
import kelelium._impl
from kelelium._impl import APIImpl
from kelelium._impl.util.html import get_easily_readable_snippet
from kelelium._impl.util.inspect_ import repr_args
...
def start_firefox(url=None, headless=False, options=None,driver_path=None):
	"""
	...
	If this doesn't work for you, then it may be that kelelium's copy of
	...
	On shutdown of the Python interpreter, kelelium cleans up all resources used
...
def start_chrome(url=None, headless=False, options=None, driver_path=None):
	"""
	:param url: URL to open.
	:type url: str
	...
	not close the browser itself. If you want to terminate the browser at the
		...
		kill_browser()
	"""
	return _get_api_impl().start_chrome_impl(url, headless, options, driver_path)
...
def set_driver(driver):
	"""
	Sets the Selenium WebDriver used to execute kelelium commands. See also
	:py:func:`get_driver`.
	"""
...
def get_driver():
	"""
	Returns the Selenium WebDriver currently used by kelelium to execute all
	commands. Each kelelium command such as ``click("Login")`` is translated to a
...
def press(key):
	...
	:py:class:`selenium.webdriver.common.keys.Keys`. kelelium makes all those keys
...
def attach_file(file_path, to=None):
	...
	parameter, then kelelium attaches the file to the first file input element it
	finds on the page.
	"""
...
def refresh():
	"""
	Refreshes the current page. If an alert dialog is open, then kelelium first
	closes it.
	"""
...
def wait_until(condition_fn, timeout_secs=10, interval_secs=0.5):
	...
	``interval_secs`` specifies the number of seconds kelelium waits between
...
class Config:
	"""
	This class contains kelelium's run-time configuration. To modify kelelium's
	...
	"""
	``implicit_wait_secs`` is kelelium's analogue to Selenium's
	``.implicitly_wait(secs)``. Suppose you have a script that executes the
	following command::

		>>> click("Download")

	If the "Download" element is not immediately available, then kelelium waits up
	...
	To disable kelelium's implicit waits, simply execute::

		Config.implicit_wait_secs = 0

	kelelium's implicit waits do not affect commands :py:func:`find_all` or
	...
	``.implicitly_wait(...)`` in conjunction with kelelium.
	"""
...
class GUIElement:
	...
	def _impl(self):
		if self._impl_cached is None:
			impl_class = \
				getattr(kelelium._impl, self.__class__.__name__ + 'Impl')
...
class HTMLElement(GUIElement):
	...
	def top_left(self):
		"""
		The top left corner of this element, as a :py:class:`kelelium.Point`.
...
class S(HTMLElement):
	...
	The parameter which you pass to ``S(...)`` is interpreted by kelelium
	according to these rules:

	 * If it starts with an ``@``, then it identifies elements by HTML ``name``.
	   Eg. ``S("@btnName")`` identifies an element with ``name="btnName"``.
	 * If it starts with ``//``, then kelelium interprets it as an XPath.
	 * Otherwise, kelelium interprets it as a CSS selector. This in particular
...
class Image(HTMLElement):
	...
	Typically, this is done via the image's alt text. For instance::
		...
		click(Image(alt="kelelium Logo"))

	You can also query an image's properties. For example, the following snippet
	clicks on the image with alt text "kelelium Logo" only if it exists::

		if Image("kelelium Logo").exists():
		    click(Image("kelelium Logo"))

	When there are multiple occurrences of an image on a page, you can
	disambiguate between them using the keyword parameters ``below``,
	``to_right_of``, ``above`` and ``to_left_of``. For instance::

		click(Image("kelelium Logo", to_left_of=ListItem("Download")))
	"""
	def __init__(
