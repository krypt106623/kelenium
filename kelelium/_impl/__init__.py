...
from kelelium._impl.match_type import PREFIX_IGNORE_CASE
from kelelium._impl.selenium_wrappers import WebElementWrapper, \
	WebDriverWrapper, FrameIterator, FramesChangedWhileIterating
from kelelium._impl.util.dictionary import inverse
from kelelium._impl.util.system import is_windows, get_canonical_os_name
from kelelium._impl.util.xpath import lower, predicate, predicate_or
...
from kelelium._impl.util.os_ import make_executable
...
class APIImpl:
	...
	def __init__(self):
		self.driver = None
	def start_firefox_impl(self, url=None, headless=False, options=None, driver_path=None):
		firefox_driver = self._start_firefox_driver(headless, options, driver_path)
		return self._start(firefox_driver, url)
	def _start_firefox_driver(self, headless, options, driver_path):
		...
		return result
	def start_chrome_impl(self, url=None, headless=False, options=None, driver_path=None):
		chrome_driver = self._start_chrome_driver(headless, options, driver_path)
		return self._start(chrome_driver, url)
	def _start_chrome_driver(self, headless, options, driver_path):
	...
	def write_impl(self, text, into=None):
		if into is not None:
			from kelelium import GUIElement
	...
	def _unwrap_clickable_element(self, elt):
		from kelelium import HTMLElement, Point
		offset = None
	...
	def highlight_impl(self, element):
		driver = self.require_driver()
		from kelelium import HTMLElement, Text
...
class DragHelper:
	...
	def _attempt_html_5_drag(self, element_to_drag):
		return self._execute_script(
			...
			"    return false;"
			"window.kelelium.dragHelper.draggedElement = draggableParent;"
			"var dragStart = window.kelelium.dragHelper.createEvent('dragstart');"
			"source.dispatchEvent(dragStart);"
			"window.kelelium.dragHelper.dataTransfer = dragStart.dataTransfer;"
			"return true;",
			element_to_drag
		)
	def _complete_html_5_drag(self, on):
		self._execute_script(
			"var target = arguments[0];"
			"var drop = window.kelelium.dragHelper.createEvent('drop');"
			"drop.dataTransfer = window.kelelium.dragHelper.dataTransfer;"
			"target.dispatchEvent(drop);"
			"var dragEnd = window.kelelium.dragHelper.createEvent('dragend');"
			"dragEnd.dataTransfer = window.kelelium.dragHelper.dataTransfer;"
			"window.kelelium.dragHelper.draggedElement.dispatchEvent(dragEnd);",
			on
		)
	def __exit__(self, *_):
		self._execute_script("delete window.kelelium;")
...
class HTMLElementImpl(GUIElementImpl):
	...
	def _unwrap_element(self, element):
		...
		from kelelium import HTMLElement
		if isinstance(element, HTMLElement):
